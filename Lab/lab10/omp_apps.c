...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i = omp_get_thread_num(); i<ARRAY_SIZE; i+=n)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i = TN  * chunks; i < (TN + 1) * chunks; i++)    z[i] = x[i] + y[i];
    if (TN + 1 == n){
      for (int i = chunks * n; i < ARRAY_SIZE; i++){
        z[i] = x[i] + y[i]; 
    }
   }
  }  
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
  ...
  #pragma omp parallel
  {
    ...
    int N = omp_get_num_threads();
    int chunks = arr_size / N;
    double sum = 0.0;
    for (int i = n * chunks; i < (n + 1) * chunks; i++)
      sum += x[i] * y[i];
    if (n == N - 1){
        for (int i = chunks * N; i < arr_size; i++){
                sum += x[i] * y[i];
        }
    }
    #pragma omp for
